...
<?php
	...
	foreach ($calendar as $key => $value) {
		...
		if (isset($_GET['today']) && $_GET['today'] = 'no') {
?>
		<td class="<?= ($cnt == 0) ? 'sunday' : '' ?><?= ($cnt == 6) ? 'saturday' : '' ?><?= $holiday_class ?><?= ((date('Y') == $year) && (date('m') == $month) && (date('d') == $value['day'])) ? '' : '' ?>"><?php $cnt++; ?><span><?= $value['day'] ?></span><?= $holidays_name ?></td>
<?php
		} else {
?>
